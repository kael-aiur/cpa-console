...
<template>
  <section class="page usage-page">
    ...
    <template v-else>
      <section class="usage-stat-grid"><article><span>总请求数</span><strong>{{ formatNumber(summary?.total_requests ?? 0) }}</strong><small>次请求</small></article><article><span>总 Token 数</span><strong>{{ formatNumber(summary?.total_tokens ?? 0) }}</strong><small>输入 + 输出 Token</small></article><article><span>平均耗时</span><strong>{{ formatDuration(summary?.average_duration_ms ?? 0) }}</strong><small>每次请求平均响应时间</small></article></section>

      <section class="usage-chart-grid">
        <article class="usage-chart-card"><header><div><span class="section-label">模型 Token 占比</span><p>按总 Token 数计算</p></div></header><div class="donut-layout"><div class="donut-chart" :style="{ background: modelGradient(summary?.model_token_distribution) }"><div><strong>{{ formatNumber(summary?.total_tokens ?? 0) }}</strong><span>Token</span></div></div><div class="chart-legend"><div v-for="(value, model, index) in summary?.model_token_distribution" :key="model"><i :style="{ background: modelColors[index % modelColors.length] }"></i><span>{{ model }}</span><strong>{{ percent(value, tokenTotal).toFixed(1) }}%</strong></div></div></div></article>
